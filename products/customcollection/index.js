...
exports.backupCustomCollection = functions.https.onCall((data) => {
  fetch("https://ariztar-sandbox.myshopify.com/admin/api/2022-10/custom_collections.json", {
    headers: {
    ...
    data.custom_collections.forEach((doc)=>{
      ...
      const docRef = db.collection("CustomCollections").doc(todayDate).collection("data").doc();
...
exports.restoreCustomCollection = functions.https.onCall((data) => {
  ...
  console.log("daata date", todayDate);
  admin.firestore().collection("CustomCollections").doc(data.todayDate).collection("data").limit(1).get().then((docs) => {
    ...
    docs.forEach((doc) => {
      ...
      fetch("https://ariztar-sandbox.myshopify.com/admin/api/2022-10/custom_collections/"+id+".json", {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          // eslint-disable-next-line camelcase, no-undef
        ...
        if (data.custom_collection) {
          ...
          fetch("https://ariztar-sandbox.myshopify.com/admin/api/2022-10/custom_collections/"+data.custom_collection.id+".json", {
            method: "PUT",
            headers: {
              "Content-Type": "application/json",
              // eslint-disable-next-line camelcase, no-undef
