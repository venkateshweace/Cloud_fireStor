...
exports.backupInventoryCustomer = functions.https.onCall((data) => {
  fetch("https://ariztar-sandbox.myshopify.com/admin/api/2022-10/customers.json", {
    ...
    //   const docRef = db.collection("Backup's").collection("customers").doc(todayDate).collection("data").doc();
    //   batch.set(docRef, doc);
    // });
    batch.commit();
  });
});
exports.restoreInventoryCustomer = functions.https.onCall((data) => {
  ...
  admin.firestore().collection("Backup's").collection("customers").doc(data.todayDate).collection("data").limit(1).get().then((docs) => {
