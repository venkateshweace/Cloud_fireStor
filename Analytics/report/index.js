...
exports.backupReports = functions.https.onCall((data) => {
  fetch("https://ariztar-sandbox.myshopify.com/admin/api/2022-10/reports.json", {
    headers: {
    ...
    data.reports.forEach((doc)=>{
      ...
      const docRef = db.collection("Backup's").doc("Reports").collection("Date:"+todayDate+"Total :"+data.reports.length).doc();
      batch.set(docRef, doc);
    });
    batch.commit();
  });
});
exports.restoreCustomer = functions.https.onCall((data) => {
  ...
  admin.firestore().collection("Backup's").doc("Reports").collection("Date:"+data.todayDate+"Total :"+data.length).limit(1).get().then((docs) => {
